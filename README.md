...
Before downgrading a diagnostic, validation deep-copies its parent in memory, moves only the known child into its strict position, and revalidates the complete subtree. For `w:uiPriority`, that position also precedes any `w:semiHidden` and `w:unhideWhenUsed` flags before `w:qFormat`. Any remaining duplicate, illegal element, invalid value, or validator failure keeps the original diagnostic blocking. The source document is never reordered or saved by this check. Malformed XML, missing relationships/resources, illegal formula structure, and every other unexpected child remain errors.
